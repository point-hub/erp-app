...
import { useUpdatePurchaseInvoiceApi } from './update.api'

const updatePurchaseInvoiceApi = useUpdatePurchaseInvoiceApi()
...
onMounted(async () => {
  ...
  if (!authStore.permission?.purchasing?.payment - orders?.read) {
...
const onSave = async () => {
  ...
  isSaving.value = true
  // check permission
  if (!authStore.permission?.purchasing?.payment - orders?.update) {
    router.push('/unauthorized')
  ...
  // api call
  const response = await updatePurchaseInvoiceApi.send(form.data, form.errors)
  if (response?.inserted_id) router.push('/purchasing/payment-orders/' + response.inserted_id)
  // state saving end
  isSaving.value = false
}
</script>
