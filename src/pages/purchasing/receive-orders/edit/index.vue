...
import { useRetrieveReceiveOrderApi } from './retrieve.api'
import { useUpdateReceiveOrderApi } from './update.api'

const updateReceiveOrderApi = useUpdateReceiveOrderApi()
...
const retrieveReceiveOrderApi = useRetrieveReceiveOrderApi()
...
onMounted(async () => {
  ...
  const response = await retrieveReceiveOrderApi.send(route.params.id.toString())
...
const onSave = async () => {
  ...
  isSaving.value = true
  // check permission
  if (!authStore.permission?.purchasing?.receive_orders?.update) {
    router.push('/unauthorized')
  ...
  // api call
  const response = await updateReceiveOrderApi.send(form.data, form.errors)
  if (response?.inserted_id) router.push('/purchasing/receive-orders/' + response.inserted_id)
  // state saving end
  isSaving.value = false
}
</script>
