...
import { useCreateReceiveOrderApi } from './create.api'
...
const createReceiveOrderApi = useCreateReceiveOrderApi()

watch(
  () => form.data.purchase_order,
  () => {
    form.data.supplier = form.data.purchase_order?.supplier
    form.data.required_date = form.data.purchase_order?.required_date
    ...
    form.data.details = form.data.details?.map((detail) => {
      return {
        ...detail,
        quantity_order: detail.quantity_pending,
...
onMounted(async () => {
  ...
  if (!authStore.permission?.purchasing?.receive_orders?.create) {
...
const onSave = async () => {
  ...
  // api call
  const response = await createReceiveOrderApi.send(form.data, form.errors)
