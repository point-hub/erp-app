...
export interface ISelectedReceiveOrder {
  _id: string
  label?: string
...
const selected = defineModel<ISelectedReceiveOrder>('selected')
...
const apiCall = async () => {
  ...
  if (response?.data) {
    options.value = response.data.map((data: ISelectedReceiveOrder) => {
      return {
        _id: data._id,
