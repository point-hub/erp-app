...
import { useUpdatePurchaseRequestApi } from './update.api'

const updatePurchaseRequestApi = useUpdatePurchaseRequestApi()
...
const onSave = async () => {
  ...
  isSaving.value = true
  // check permission
  if (!authStore.permission?.purchasing?.purchase_requests?.update) {
    router.push('/unauthorized')
    return
  }
  ...
  // api call
  const response = await updatePurchaseRequestApi.send(form.data, form.errors)
