...
const onApprove = async () => {
  ...
  toastRef.toast(`Approve Down Payment "${data.value.form_number}" success`, {
    color: 'success'
  })
  ...
  router.push('/purchasing/down-payments')
}
...
<template>
  <base-card class="py-4!">
    <div class="flex flex-wrap gap-2">
      ...
      <router-link
        v-if="authStore.permission?.purchasing?.down_payments?.update && !data.is_deleted"
        :to="`/purchasing/down-payments/${route.params.id}/edit`"
      >
        <base-button color="info" size="sm">
      ...
      <!-- <router-link
        v-if="authStore.permission?.purchasing?.down_payments?.update"
        :to="`/purchasing/down-payments/${route.params.id}/edit`"
      >
        <base-button color="info" size="sm">
      ...
      <base-button
        v-if="authStore.permission?.purchasing?.down_payments?.delete && !data.is_deleted"
        color="danger"
        size="sm"
        @click="
      ...
      <base-button
        v-if="
          authStore.permission?.purchasing?.down_payments?.approval &&
