...
import { useDeleteUserApi } from './delete-user.api'
...
const onDelete = async () => {
  ...
  const responseDelete = await deleteUserApi.send(id.value, reason.value, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef.toast(`Delete User "${name.value} @${username.value}" success`, { color: 'success' })
  toggleModal(false)
    ...
    <div class="max-h-90vh overflow-auto p-4">
      ...
      <div class="space-y-8">
        ...
        <div class="flex flex-col">
          ...
          <span>{{ name }}</span>
          <span class="font-light text-xs">@{{ username }}</span>
        </div>
        <base-textarea
          required
