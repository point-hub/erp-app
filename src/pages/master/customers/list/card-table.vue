...
const updateRouter = () => {
  router.push({
    ...
    query: {
      ...
      'search.customer_group': search.value.customer_group,
...
watchDebounced(
  ...
  async () => {
    ...
    // call api
    const response = await getCustomersApi.send(
...
onMounted(async () => {
  ...
  search.value.customer_group = route.query['search.customer_group']?.toString() ?? ''
...
const onDeleteModal = (customer_group: ICustomer, index: number) => {
  rowMenuRef.value[index].toggle(false)
  deleteModalRef.value.toggleModal(true, {
    id: customer_group._id,
    name: `[${customer_group.code}] ${customer_group.name}`
  })
}
                    ...
                    <base-card class="py-1! px-2! text-sm">
                      <div class="flex flex-col">
                        ...
                        <base-button
                          ...
                          variant="text"
                          color="danger"
                          @click="onDeleteModal(customer, index)"
                        >
                          <div class="flex gap-2 w-full">
