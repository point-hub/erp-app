...
const options = defineModel<ISelectedCustomer[]>('options')
const required = defineModel<boolean>('required', { default: false })
const selected = defineModel<ISelectedCustomer>('selected')
const label = defineModel<string>('label', { default: 'Customer' })
const errors = ref<string[]>([])

const getCustomersApi = useGetCustomersApi()
const search = ref('')
const isLoading = ref<boolean>(false)
const localOptions = ref<ISelectedCustomer[]>()

const apiCall = async () => {
  const response = await getCustomersApi.send(search.value, 1)
  if (response?.data) {
    options.value = response.data.map((data: ISelectedCustomer) => {
      return {
        _id: data._id,
        label: data.label,
        code: data.code,
