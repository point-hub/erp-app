...
const getSupplierGroups = async () => {
  ...
  supplierGroups.value = response.data.data
  pagination.value = response.data.pagination
}
const rowMenuRef = ref()
const supplierGroups = ref<ISupplierGroup[]>()
...
const openMenu = (supplierGroup: ISupplierGroup, index: number) => {
  rowMenuRef.value[index].toggle(false)
  deleteModalRef.value.toggleModal(true, {
    id: supplierGroup._id,
...
<template>
  <div class="flex flex-col gap-4">
    <card-breadcrumbs />
    <base-card>
      <template #header>Supplier Groups</template>
      <div class="my-5 flex gap-2">
        <router-link to="/master/supplier-groups/create">
        ...
        <base-table>
          ...
          <tbody>
            ...
            <template v-if="!isLoading">
              <tr v-for="(supplierGroup, index) in supplierGroups" :key="index">
                <td>
                    ...
                    <template #content>
                      <base-card class="py-1! px-2! text-sm">
                        <div class="flex flex-col">
                          <router-link :to="`/master/supplier-groups/${supplierGroup._id}`">
                          ...
                          <base-button
                            variant="text"
                            color="danger"
                            @click="openMenu(supplierGroup, index)"
                          >
                            <div class="flex gap-2 w-full">
                ...
                <td>
                  <router-link
                    :to="`/master/supplier-groups/${supplierGroup._id}`"
                    class="text-blue"
                  >
                    {{ supplierGroup.code }}
                  </router-link>
                ...
                <td>{{ formatDate(new Date(supplierGroup.created_date), 'dd-MM-yyyy') }}</td>
              </tr>
            </template>
          </tbody>
        </base-table>
