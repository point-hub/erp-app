...
import { useDeleteSupplierGroupApi } from './delete.api'
...
const onDelete = async () => {
  ...
  // start api call
  const deleteSupplierGroupApi = useDeleteSupplierGroupApi()
  const responseDelete = await deleteSupplierGroupApi.send(id.value, reason.value, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef.toast(`Delete Supplier Group "${name.value}" success`, { color: 'success' })
  toggleModal(false)
