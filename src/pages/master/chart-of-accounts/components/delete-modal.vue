...
const onDelete = async () => {
  ...
  // start api call
  try {
    const response = await axios.post(`/v1/master/chart-of-accounts/${id.value}/delete`, {
      reason: reason.value
    })
    if (response.status === 200) {
      ...
      password.value = ''
      reason.value = ''
      toastRef.toast(`Delete Chart of Account "${name.value}" success`, {
        color: 'success'
      })
      toggleModal(false)
    }
...
<template>
  <base-modal :is-open="showModal" @on-close="toggleModal(false)">
    <div class="max-h-90vh overflow-auto p-4">
      <h2 class="py-4 text-2xl font-bold">Delete Chart of Account</h2>
      <div class="space-y-8">
        <p>
          Please enter your reason and password to protect you from accidentally deleting your data
        </p>
        <div class="flex flex-col">
          <span class="font-semibold">Chart of Account</span>
          <span>{{ name }}</span>
        </div>
        <base-textarea
          required
