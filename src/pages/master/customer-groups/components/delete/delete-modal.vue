...
import { useDeleteCustomerGroupApi } from './delete.api'
...
const onDelete = async () => {
  ...
  // start api call
  const deleteCustomerGroupApi = useDeleteCustomerGroupApi()
  const responseDelete = await deleteCustomerGroupApi.send(id.value, reason.value, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef.toast(`Delete Customer Group "${name.value}" success`, { color: 'success' })
  toggleModal(false)
...
<template>
  <base-modal :is-open="showModal" @on-close="toggleModal(false)">
    <div class="max-h-90vh overflow-auto p-4">
      <h2 class="py-4 text-2xl font-bold">Delete Customer Group</h2>
      <div class="space-y-8">
        <p>
          Please enter your reason and password to protect you from accidentally deleting your data
        </p>
        <div class="flex flex-col">
          <span class="font-semibold">Customer Group</span>
          <span>{{ name }}</span>
        </div>
        <base-textarea
          required
