...
import { useDeleteItemApi } from './delete.api'
...
const onDelete = async () => {
  ...
  // start api call
  const deleteItemApi = useDeleteItemApi()
  const responseDelete = await deleteItemApi.send(id.value, reason.value, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef.toast(`Delete Item "${name.value}" success`, { color: 'success' })
  toggleModal(false)
...
<template>
  <base-modal :is-open="showModal" @on-close="toggleModal(false)">
    <div class="max-h-90vh overflow-auto p-4">
      <h2 class="py-4 text-2xl font-bold">Delete Item</h2>
      <div class="space-y-8">
        <p>
