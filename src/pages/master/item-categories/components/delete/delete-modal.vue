...
import { useDeleteItemCategoryApi } from './delete.api'
...
const onDelete = async () => {
  ...
  // start api call
  const deleteItemCategoryApi = useDeleteItemCategoryApi()
  const responseDelete = await deleteItemCategoryApi.send(id.value, reason.value, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef.toast(`Delete Item Category "${name.value}" success`, { color: 'success' })
  toggleModal(false)
